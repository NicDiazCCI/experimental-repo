...
export function randomDelay(min: number = 100, max: number = 1000, randomFn: () => number = Math.random, setTimeoutFn: (cb: (...args: any[]) => void, ms: number) => any = setTimeout): Promise<void> {
  const delay = Math.floor(randomFn() * (max - min + 1)) + min;
  return new Promise(resolve => setTimeoutFn(resolve, delay));
}

export function flakyApiCall(randomFn: () => number = Math.random, setTimeoutFn: (cb: (...args: any[]) => void, ms: number) => any = setTimeout): Promise<string> {
